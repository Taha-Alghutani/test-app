...
# أمر بدء التشغيل
#CMD ["sh", "-c", "python wait_for_postgres.py && python manage.py migrate && gunicorn testtest.wsgi:application --bind 0.0.0.0:8000"]
...
CMD ["sh", "-c", "\
     ...
     gunicorn testtest.wsgi:application --bind 0.0.0.0:8000"]
